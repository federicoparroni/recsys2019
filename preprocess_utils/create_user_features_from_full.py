...
def extract_features_from_full(mode, cluster='no_cluster'):
    ...
    feat_label = pd.read_csv('dataset/preprocessed/no_cluster/local/feature/impression_label/features.csv')
    ...
    features_full = features_full[~features_full.session_id.isin(sessions_to_correct)]
    features_full = pd.concat([features_full, to_add])
    features_full = features_full.sort_values(by=['user_id', 'session_id'])
