...
class ClusterizeBase(ABC):
    ...
    def save(self):
        ...
        set_target_indices = set(self.test_indices)
        ...
        test_local_df.loc[valid_test_indices].to_csv(os.path.join(local_path, 'test.csv'))
        if len(self.target_indices) > 0:
        ...
        test_small_df.loc[valid_test_indices].to_csv(os.path.join(small_path, 'test.csv'))
        if len(self.target_indices) > 0:
