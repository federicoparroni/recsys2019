...
def progress(bytes_so_far, bytes_to_be_transferred):
    print('progress: {}%'.format((bytes_so_far/bytes_to_be_transferred)*100), end="\r")
...
def download_scores_and_sub(pattern, user_name = 'ubuntu'):
    path_to_pem = easygui.fileopenbox(msg='pick the pem', default='~/Downloads')
    ...
    for afile in filelist:
        ...
        sftp.get(filename, join(downloads_folder, pattern,filename.split('/')[-1]), progress)
