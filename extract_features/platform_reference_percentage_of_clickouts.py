...
class PlatformReferencePercentageOfClickouts(FeatureBase):

    """
    This feature for each impression of the clickout says the percentage of
    clickouts made on a platform on that impression.
    Example: if the platform of the clickout session is IT and there are overall
    100 clickouts in IT, for the impression number 3, if it has been clicked
    20 times in IT, the feature value for the impression 3 will be 0.2 (that means
    that 20% of all the clickouts in IT have been with impression 3).

    user_id | session_id | item_id | percentage_of_total_plat_clk

    """

    def __init__(self, mode, cluster='no_cluster'):
        name = 'platform_reference_percentage_of_clickouts'
        super(PlatformReferencePercentageOfClickouts, self).__init__(
    ...
    def extract_feature(self):
        ...
        final_df['percentage_of_total_plat_clk'] = 0.0
        for t in zip(final_df.index, final_df.n_interactions_per_item, final_df.n_interactions_per_plat):
            percentage_of_total_plat_clk = round((t[1] * 100.0) / t[2],2)
            final_df.at[t[0], 'percentage_of_total_plat_clk'] = percentage_of_total_plat_clk

        feature = final_df[['platform','item_id','percentage_of_total_plat_clk']]
        clk_expanded = expand_impressions(clickout_rows)
        feature = pd.merge(clk_expanded, feature, how='left', on=['platform','item_id']).fillna(0)
        feature = feature[['user_id','session_id','item_id','percentage_of_total_plat_clk']]
...
if __name__ == '__main__':
    ...
    c = PlatformReferencePercentageOfClickouts(mode, cluster)
