...
class AdjustedLocationReferencePercentageOfInteractions(FeatureBase):

    """
    Same feature as LocationReferencePercentageOfInteractions, but the popularity
    is calculated in a slightly different way as in PersonalizedTopPop.

    user_id | session_id | item_id | adj_percentage_of_total_city_inter

    """

    def __init__(self, mode, cluster='no_cluster'):
        name = 'adjusted_location_reference_percentage_of_interactions'
        super(AdjustedLocationReferencePercentageOfInteractions, self).__init__(
    ...
    def extract_feature(self):
        ...
        clickout_rows = df.loc[last_clickout_indices,
            ['user_id','session_id','city','reference','impressions']][df.action_type == 'clickout item']
        ...
        merged_df = pd.merge(df_item_clicks, df_city_clicks, how='left', on=['city']).fillna(0)
        clk_expanded = expand_impressions(clickout_rows)
        feature = pd.merge(clk_expanded, merged_df, how='left', on=['item_id','city']).fillna(0)
        ...
        for t in tqdm(zip(feature.reference, feature.item_id,
                          feature.n_interactions_per_item, feature.n_interactions_per_city)):
            if t[0] == t[1]: # è quello cliccato
                if t[3] != 1:
                    percentage_of_total_city_clk = round(((t[2]-1) * 100.0) / (t[3]-1),5)
                else:
                    percentage_of_total_city_clk = 0
            else: # non è quello cliccato
                if 0 not in [t[2],t[3]] and t[3]!=1:
                    percentage_of_total_city_clk = round((t[2] * 100.0) / (t[3]-1),5) # tolgo comunque il click per plat
                else:
                    percentage_of_total_city_clk = 0
            new_col.append(percentage_of_total_city_clk)
        feature['adj_percentage_of_total_city_inter'] = new_col
        feature.adj_percentage_of_total_city_inter = feature.adj_percentage_of_total_city_inter.astype(float)
        final_feature_reduced = feature[['user_id','session_id','item_id','adj_percentage_of_total_city_inter']]
...
if __name__ == '__main__':
    ...
    c = AdjustedLocationReferencePercentageOfInteractions(mode, cluster)
