...
class User2Item(FeatureBase):
    ...
    def __init__(self, mode, cluster='no_cluster', repeated_reference=False):
        self.repeated_reference=repeated_reference
        name = f'user_2_item_{repeated_reference}'
    ...
    def extract_feature(self):
        ...
        # create a diction with keys tuples like ('user_id', 'session_id') and as value the list of the references
        # in the session ['ref1', 'ref5', ... 'ref9']
        sess_user_dict = {}
        ...
        for idx in tqdm(idxs_clicks):
            ...
            features_imp = icm.loc[impressions]
        ...
        return pd.DataFrame(final_list, columns=['user_id', 'session_id', 'item_id', 'sim_score'])
