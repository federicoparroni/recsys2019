...
class ScoresXGBoostDanParamUnbiased(FeatureBase):
    ...
    def __init__(self, mode, cluster='no_cluster'):
        name = 'scores_xgboost_danparam_unbiased'
        super(ScoresXGBoostDanParamUnbiased, self).__init__(
            name=name, mode=mode, cluster=cluster
            )

    def extract_feature(self):
        print('cannot create this features! just move the file feature.csv from drive to the folder \
               \'mode/feature/scores_xgboost_danparam_unbiased/' )
    ...
    def read_feature(self, one_hot=False, create_not_existing_features=True):
        ...
        df['normalized_xgb_unbias'] = x_scaled
        df = df[['user_id', 'session_id', 'item_id', 'normalized_xgb_unbias']]
